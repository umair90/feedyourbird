...
function bird_scene()
    ...
    self.box3 = director:createSprite( {
     ...
     physics:addNode(self.box1, {density = 3})  
     physics:addNode(self.box2, {density = 4})
     physics:addNode(self.box3, {density = 5})
...
function select_level_2()
  director:moveToScene(level_selector,{ transitionType = "fadeUp", transitionTime = 0.5 })
  physics:resume()
end

function next_level_2()
  director:moveToScene(level_3,{ transitionType = "fadeUp", transitionTime = 0.5 })
  physics:resume()
end  
function getSuspend_2(event)
  if event.phase == "began" then
...
function getResume_2()
  
    self.bg.isTouchable = true;
  --  self.bg:addEventListener("touch", moveCrate) 
    self.button_pause2:addEventListener("touch", getSuspend_2)
...
director:getCurrentScene():releaseAtlas(self.box3.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.box3.animation)
