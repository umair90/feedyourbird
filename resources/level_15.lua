...
function bird_scene()
      ...
      self.player = director:createSprite( {
          ...
          self.rm = director:createRectangle( {
              x=dw*1/6, y=dh*3/3.2, w=dw/8, h=dh/30,
              source="graphics/stripLarge.png",
              xAnchor=0.5, yAnchor=0.5,
              strokeWidth=4, color=color.brown,
              zOrder=1,
            } )
            physics:addNode(self.rm)
                                                          -- right solid pillar
      self.leftTyre = director:createSprite( {

                name = "slope",
                x=self.rm.x-self.rm.w/2+1, y=self.rm.y-40,
                source="graphics/wheel.png",
                xAnchor=0.5, yAnchor=0.5,
                rotation = 90,
                } )
                                                           -- left solid pillar
      self.rightTyre = director:createSprite( {
                name = "slope",
                 x=self.rm.x+self.rm.w/2-1, y=self.rm.y-40,
                source="graphics/wheel.png",
                xAnchor=0.5, yAnchor=0.5,
        ...
        self.upperLeftPillar=  director:createSprite( {
                name = "slope",
                 x=120, y=350,
                source="graphics/stripLarge.png",
                xAnchor=0.5, yAnchor=0.5,
         ...
         self.upperSlopePillar=  director:createSprite( {
                name = "slope",
                 x=self.upperLeftPillar.w+120, y=340,
                source="graphics/stripLarge.png",
                xAnchor=0.5, yAnchor=0.5,
                rotation = 5,
                } )
            
            self.lowerSlopePillar=  director:createSprite( {
                name = "slope",
                 x=self.upperLeftPillar.w+280, y=200,
                source="graphics/stripLarge.png",
                xAnchor=0.5, yAnchor=0.5,
                rotation = -15,
                } )
      ...
      self.leftPillar = director:createSprite( {
            ...
            self.Wheel = director:createSprite( {
                name = "slope",
                x = 368, y = 230,
   ...
   physics:addNode(self.leftTyre, {density = 15,radius=38})
   physics:addNode(self.rightTyre, {density = 10,radius=38})
   physics:addNode(self.player)	
   physics:addNode(self.upperLeftPillar, {type="static"})	
   physics:addNode(self.upperSlopePillar, {type="static"})	
   physics:addNode(self.lowerSlopePillar, {type="static"})	
   physics:addNode(self.leftPillar, {type="static"})	
   physics:addNode(self.rightPillar, {type="static"})	
   physics:addNode(self.Wheel,{density = 1.5,radius=42})
   local j1 = physics:createWheelJoint(self.rm, self.leftTyre, 0, 1, self.leftTyre.x, self.rm.y, self.leftTyre.x, self.leftTyre.y)
   local j2 = physics:createWheelJoint(self.rm, self.rightTyre, 0, 1, self.rightTyre.x, self.rm.y, self.rightTyre.x, self.rightTyre.y)
   -- Remove suspension
   j1.frequency=5
   j2.frequency=5
...
physics:removeNode(self.rm)
physics:removeNode(self.player)
physics:removeNode(self.bar)
physics:removeNode(crate_1)
physics:removeNode(self.leftTyre)
physics:removeNode(self.rightTyre)
physics:removeNode(self.upperLeftPillar)
physics:removeNode(self.upperSlopePillar) 
physics:removeNode(self.lowerSlopePillar)
physics:removeNode(self.rightPillar)
physics:removeNode(self.leftPillar)
physics:removeNode(self.Wheel)
                                                             --Remove variables
self.player:removeFromParent()
self.rm:removeFromParent()
self.bar:removeFromParent()
self.bg:removeFromParent()
crate_1:removeFromParent()
self.leftTyre:removeFromParent()
self.rightTyre:removeFromParent()
self.upperSlopePillar:removeFromParent()
self.upperLeftPillar:removeFromParent()
self.lowerSlopePillar:removeFromParent()
...
---------------------------------
director:getCurrentScene():releaseAtlas(self.leftTyre.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.leftTyre.animation)

director:getCurrentScene():releaseAtlas(self.rightTyre.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.rightTyre.animation)

director:getCurrentScene():releaseAtlas(self.upperSlopePillar.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.upperSlopePillar.animation)

director:getCurrentScene():releaseAtlas(self.upperLeftPillar.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.upperLeftPillar.animation)

director:getCurrentScene():releaseAtlas(self.lowerSlopePillar.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.lowerSlopePillar.animation)
...
director:getCurrentScene():releaseAtlas(self.Wheel.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.Wheel.animation)
...
self.leftTyre = nil
self.rightTyre = nil
self.upperLeftPillar=nil
self.upperSlopePillar=nil    
self.lowerSlopePillar=nil
self.rm=nil
