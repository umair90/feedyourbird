...
dbg.print("level1 setup scene started")
scoreHigh=get_highScore14()
set_level(14)
set_stage(4)
...
function bird_scene()
      ...
      self.player = director:createSprite( {
       ...
       local triPoints = {0,0, 95,0, 48,81}
      local tri = director:createSprite(director.displayCenterX, 143, "graphics/triangle.png")
      tri.xAnchor=0.5; tri.yAnchor=1
      physics:addNode(tri, {shape=triPoints,type="static"})
      self.largeStrip = director:createSprite( {
                x=director.displayCenterX-0.5, y=160,
                source="graphics/stripLargeStatic.png",
                xAnchor=0.5, yAnchor=0.5,
                rotation=0,
                } )
       self.brickLeft = director:createSprite( {
                x=300, y=100,
                source="graphics/brick.png",
                xAnchor=0.5, yAnchor=0.5,
                rotation=90,
                } )
        physics:addNode(self.player)	
        physics:addNode(self.largeStrip)
        physics:addNode(self.brickLeft)
...
function select_level_14()
  director:moveToScene(level_selector,{ transitionType = "fadeUp", transitionTime = 0.5 })
  physics:resume()
end

function next_level_14()
  director:moveToScene(level_15,{ transitionType = "fadeUp", transitionTime = 0.5 })
  physics:resume()
end  
...
physics:removeNode(self.largeStrip)
physics:removeNode(self.brickLeft)
                                                             --Remove variables
...
self.largeStrip:removeFromParent()
---------------------------------------------------------
...
director:getCurrentScene():releaseAtlas(self.brickLeft.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.brickLeft.animation)

director:getCurrentScene():releaseAtlas(self.largeStrip.animation.usedAtlases)
director:getCurrentScene():releaseAnimation(self.largeStrip.animation)
